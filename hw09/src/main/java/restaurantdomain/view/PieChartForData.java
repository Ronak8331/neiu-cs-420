...
public class PieChartForData {
    ...
    private void initializeCuisinePieChart()
    {
        ...
        IntStream.range(0, distinctCuisine.size()).forEach(i -> {
            String month = distinctCuisine.get(i);
            cuisineData[i] = new javafx.scene.chart.PieChart.Data(month, restaurantList.stream().filter(days -> days.getCuisine().equals(month)).count());
        });
    ...
    public javafx.scene.chart.PieChart getCuisinePieChart() { return cuisinePieChart;}
}
